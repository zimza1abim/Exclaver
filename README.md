...
프록시가 이미 실행 중이면 저장 후 서비스를 자동으로 다시 불러와 변경사항을 바로 적용합니다.

이미 설정된 상태에서는 도메인 관리가 먼저 보입니다. conf 설정은 필요할 때만 `경로 conf 설정 보기`를 눌러 열면 됩니다.

## 프록시 포트와 설정 동기화

Smart Route는 별도 프록시 포트를 따로 저장하지 않습니다. 앱의 일반 설정에 있는 SOCKS/HTTP 인바운드 설정을 그대로 사용합니다.

- SOCKS 포트: `설정 > SOCKS5 프록시 포트`
- HTTP 포트: `설정 > HTTP 프록시 포트`
- SOCKS/HTTP 활성화 여부도 앱 설정을 따릅니다.
- `스마트 라우트 저장/적용`을 누르면 프록시 모드, SOCKS 인바운드, HTTP 인바운드가 자동으로 켜집니다.
- 설정에서 포트를 바꾸면 Smart Route 실행 시 현재 포트가 자동으로 적용됩니다.
- Smart Route 화면에는 현재 적용될 SOCKS/HTTP 주소가 표시됩니다.

예를 들어 HTTP 프록시 포트가 `2081`이면 AdGuard 같은 앱에는 `127.0.0.1:2081`을 넣으면 됩니다. `외부 접근 허용`을 켠 경우에는 화면에 `0.0.0.0:포트`로 표시됩니다.
